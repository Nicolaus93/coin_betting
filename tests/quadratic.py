if __name__ == "__main__":
    ...
    def load_args():
        parser = argparse.ArgumentParser(description='Optimizing a quadratic function')
    ...
    def main():
        ...
        if args.optim_method == 'SGD':
            optimizer = SGD(params=[x], lr=args.lr, momentum=args.momentum,
                            weight_decay=args.weight_decay, nesterov=args.nesterov)
        elif args.optim_method == 'Adam':
            optimizer = Adam(params=[x], lr=args.lr, betas=(args.beta1, args.beta2),
                             weight_decay=args.weight_decay)
        elif args.optim_method == 'SGDOL':
            optimizer = SGDOL(params=[x], smoothness=args.smoothness,
        ...
        M = torch.tensor([9.0050, -5.1853, -2.6774, -0.6063, -4.5736,  1.7690,  5.1730, -0.7051,  3.5950, -5.5852,
                          -5.1853, 16.0687, -1.7444,  1.6228,  4.9226, -6.6171, -3.3201,  6.3262,  8.4802, 11.5141,
                          -2.6774, -1.7444,  9.7646,  4.1116,  3.3880,  0.7299,  1.5001, -1.3030, -8.5188, -5.1559,
                          -0.6063,  1.6228,  4.1116, 16.6046,  1.1656, -1.8692, -0.4318, -5.6672,  0.8409, -2.5752,
                          -4.5736,  4.9226,  3.3880,  1.1656, 14.3299, -3.7571, -2.5134,  2.6684, -4.0715, 11.4973,
                          1.7690, -6.6171,  0.7299, -1.8692, -3.7571,  9.7356,  3.0703, -2.8832, -2.8462, -6.1807,
                          5.1730, -3.3201,  1.5001, -0.4318, -2.5134,  3.0703,  6.4093, -0.8201,  0.9782, -7.6011,
                          -0.7051,  6.3262, -1.3030, -5.6672,  2.6684, -2.8832, -0.8201,  8.1158,  4.3013,  7.6151,
                          3.5950,  8.4802, -8.5188,  0.8409, -4.0715, -2.8462,  0.9782,  4.3013, 19.3981,  8.3670,
                          -5.5852, 11.5141, -5.1559, -2.5752, 11.4973, -6.1807, -7.6011,  7.6151,  8.3670, 25.8494])
